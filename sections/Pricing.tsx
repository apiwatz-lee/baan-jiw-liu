      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 flex flex-col gap-3">
          ...
          <p className="text-sm sm:text-base text-[#6B4F3A]/70 leading-relaxed font-medium">
            โปร่งใส ไม่มีค่าส่วนกลางจุกจิก สัญญายืดหยุ่นเป็นธรรม มอบสิ่งอำนวยความสะดวกที่คุ้มค่าที่สุด
          ...
          {ROOMS.map((room, index) => {
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-[36px] bg-white p-8.5 shadow-md flex flex-col justify-between transition-all duration-500 border ${
                  isPopular
              ...
              >
                {/* Popular Ribbon */}
                {isPopular && (
                  <div className="absolute -top-4.5 left-1/2 -translate-x-1/2 bg-[#C8A97E] text-white text-xs font-bold tracking-widest uppercase px-5 py-2 rounded-full shadow-md flex items-center gap-1.5 border border-[#C8A97E]/30">
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    <h3 className="text-2xl font-black text-[#6B4F3A]">{room.name}</h3>
                  </div>
                  ...
                  <ul className="flex flex-col gap-3.5 mb-8">
                    {room.features.slice(0, 5).map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-[#6B4F3A]/85">
        ...
        >
          <GlassCard hoverEffect={false} className="p-8 max-w-4xl mx-auto rounded-[32px] border-none bg-white/70">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-left">
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold text-[#6B4F3A]">{OTHER_FEES.water}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-[#6B4F3A]/60 font-bold uppercase tracking-wider mb-1">
                  ค่ากระแสไฟฟ้า
                </span>
                <span className="text-sm font-bold text-[#6B4F3A]">{OTHER_FEES.electricity}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-[#6B4F3A]/60 font-bold uppercase tracking-wider mb-1">
                  ความเร็วอินเทอร์เน็ต
                </span>
                <span className="text-sm font-bold text-[#6B4F3A]">{OTHER_FEES.wifi}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-[#6B4F3A]/60 font-bold uppercase tracking-wider mb-1">
                  ค่าส่วนกลางรายเดือน
            ...
            <div className="mt-6 border-t border-[#D8C4B6]/20 pt-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#6B4F3A]/70 font-semibold">
              <span>* เงินประกันสัญญาเพียง 1 เดือน + ค่าเช่าล่วงหน้า 1 เดือน พร้อมเข้าอยู่ได้เลย</span>
