          ...
          {TESTIMONIALS.map((t, index) => (
            <motion.div key={t.id} variants={itemVariants} className="flex h-full">
              ...
              >
                ...
                {/* Profile Meta */}
                <div className="flex items-center gap-4.5 border-t border-[#D8C4B6]/20 pt-6">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-bold text-[#6B4F3A]">{t.name}</span>
                    <span className="text-xs text-[#C8A97E] font-semibold">{t.role}</span>
                  </div>
                </div>
              </GlassCard>
            </motion.div>
